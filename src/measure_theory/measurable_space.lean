...
lemma generate_from_of_finite_of_sup_closed (S : set (set α)) (hcompl : ∀ s ∈ S, sᶜ ∈ S)
  (hempty : ∅ ∈ S) (hfin : finite S) (hsup : sup_closed S) :
  generate_measurable S = S :=
let m : measurable_space α :=
{ measurable_set' := S,
  measurable_set_empty := hempty,
  measurable_set_compl := hcompl,
  measurable_set_Union := supr_mem_of_sup_closed_of_finite hsup hfin } in
...
generate_from_of_finite_of_sup_closed _ (by simp) (by simp) (by simp)
  (sup_closed_generate_from_set s)
