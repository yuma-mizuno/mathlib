...
meta def target_name (tactic_name : string) (src tgt : name) (dict : name_map name)
  (guess_name : string → string) (allow_auto_name : bool) :
  tactic name :=
(if tgt.get_prefix ≠ name.anonymous ∨ allow_auto_name -- `tgt` is a full name
 then pure tgt
      ...
      | (name.mk_string s pre) :=
        do let tgt_auto := guess_name s,
           guard (tgt.to_string ≠ tgt_auto ∨ tgt = src)
             <|> trace ("`" ++ tactic_name ++ " " ++ src.to_string ++
               "`: correctly autogenerated target " ++
      ...
      | _ := fail (tactic_name ++ ": can't transport " ++ src.to_string)
      end) >>=
(λ res,
  if res = src ∧ tgt ≠ src
  then fail (tactic_name ++ ": can't transport " ++ src.to_string ++ " to itself.
...
{ name      := `to_additive,
  ...
  after_set := some $ λ src prio persistent, do
    ...
    tgt ← target_name "to_additive" src val.tgt dict guess_name val.allow_auto_name,
