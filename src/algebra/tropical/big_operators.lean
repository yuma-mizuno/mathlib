...
lemma multiset.trop_inf [h : linear_order R] [h' : order_top R] (s : multiset R) :
  (by { haveI : semilattice_inf_top R := { ..@lattice_of_linear_order _ h, ..h'}, exact trop s.inf}) =
    multiset.sum (s.map trop) :=
begin
...
lemma finset.trop_inf [h : linear_order R] [h' : order_top R] (s : finset S) (f : S → R) :
  (by { haveI : semilattice_inf_top R := { ..@lattice_of_linear_order _ h, ..h'}, exact trop (s.inf f)}) =
  ∑ i in s, trop (f i) :=
begin
  cases s,
...
lemma trop_Inf_image {α} [conditionally_complete_linear_order R] {s : finset α}
  (f : α → with_top R) : trop (Inf (f '' s)) = ∑ i in s, trop (f i) :=
begin
