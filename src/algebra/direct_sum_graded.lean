...
additive-graded ring. The typeclasses are:
...
If the `A i` are disjoint, these provide a gradation of `⨆ i, A i`.
...
(one_mul {i} (a : A i) : (⟨_, ghas_mul.mul ghas_one.one a⟩ : Σ i, A i) = ⟨i, a⟩)
(mul_one {i} (a : A i) : (⟨_, ghas_mul.mul a ghas_one.one⟩ : Σ i, A i) = ⟨i, a⟩)
...
(mul_comm {i j} (a : A i) (b : A j) : (⟨_, ghas_mul.mul a b⟩ : Σ i, A i) = ⟨_, ghas_mul.mul b a⟩)
...
/-- The `semiring` structure derived from `semiring_add_gradation A`. -/
...
/-- The `ring` derived from `ring_add_gradation A`. -/
...
/-- The `comm_ring` derived from `ring_add_gradation A`. -/
