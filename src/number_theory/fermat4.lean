...
follows. -/
def fermat_42 (a b c : ℤ) : Prop := a ≠ 0 ∧ b ≠ 0 ∧ c ≠ 0 ∧ a ^ 4 + b ^ 4 = c ^ 2

lemma fermat_42_comm {a b c : ℤ} :
 (fermat_42 a b c) ↔ (fermat_42 b a c) :=
 by { delta fermat_42, rw add_comm, tauto }

lemma fermat_42_mul {a b c k : ℤ} (hk0 : k ≠ 0) :
...
begin
  ...
  { intro f42,
    ...
    split, { apply mul_ne_zero, { rw pow_two, exact mul_ne_zero hk0 hk0 }, exact f42.2.2.1 },
    { calc (k * a) ^ 4 + (k * b) ^ 4
          = k ^ 4 * (a ^ 4 + b ^ 4) : by ring
      ... = k ^ 4 * c ^ 2 : by rw f42.2.2.2
      ... = (k ^ 2 * c) ^ 2 : by ring }},
  { intro f42,
    split, { exact right_ne_zero_of_mul f42.1 },
    split, { exact right_ne_zero_of_mul f42.2.1 },
    split, { exact right_ne_zero_of_mul f42.2.2.1 },
    apply (mul_right_inj' (pow_ne_zero 4 hk0)).mp,
    { calc k ^ 4 * (a ^ 4 + b ^ 4)
          = (k * a) ^ 4 + (k * b) ^ 4 : by ring
      ... = (k ^ 2 * c) ^ 2 : by rw f42.2.2.2
      ... = k ^ 4 * c ^ 2 : by ring } }
end
...
lemma exists_minimal_fermat_42 {a b c : ℤ} (h : fermat_42 a b c) :
  ∃ (a0 b0 c0), (minimal_fermat_42 a0 b0 c0) :=
begin
...
lemma coprime_of_minimal_fermat_42 {a b c : ℤ} (h : minimal_fermat_42 a b c) :
  int.gcd a b = 1 :=
begin
  ...
  rw mul_lt_mul_right (nat.pos_of_ne_zero (int.nat_abs_ne_zero_of_ne_zero hf.right.right.left)),
...
begin
  rintros ⟨h1, h2⟩,
  split, { exact fermat_42_comm.mp h1 },
  exact h2
end

/-- We can assume that a minimal solution to `a ^ 4 + b ^ 4 = c ^ 2` has positive `c`. -/
lemma neg_of_minimal_fermat_42 {a b c : ℤ} :
 (minimal_fermat_42 a b c) → (minimal_fermat_42 a b (-c)) :=
begin
  rintros ⟨⟨ha, hb, hc, heq⟩, h2⟩,
  split,
  { apply and.intro ha (and.intro hb (and.intro (neg_ne_zero.mpr hc) _)),
...
lemma exists_odd_minimal_fermat_42 {a b c : ℤ} (h : fermat_42 a b c) :
  ∃ (a0 b0 c0), (minimal_fermat_42 a0 b0 c0) ∧ a0 % 2 = 1 :=
begin
  obtain ⟨a0, b0, c0, hf⟩ := exists_minimal_fermat_42 h,
  ...
  { cases int.mod_two_eq_zero_or_one b0 with hbp hbp,
    { exfalso,
      ...
      { exact int.dvd_gcd (int.dvd_of_mod_eq_zero hap) (int.dvd_of_mod_eq_zero hbp) },
        rw (coprime_of_minimal_fermat_42 hf) at h1, revert h1, norm_num },
      { exact ⟨b0, ⟨a0, ⟨c0, minimal_fermat_42_comm hf, hbp⟩⟩⟩ }},
  exact ⟨a0, ⟨b0, ⟨c0 , hf, hap⟩⟩⟩,
end

/-- We can assume that a minimal solution to `a ^ 4 + b ^ 4 = c ^ 2` has
`a` odd and `c` positive. -/
lemma exists_pos_odd_minimal_fermat_42 {a b c : ℤ} (h : fermat_42 a b c) :
  ∃ (a0 b0 c0), (minimal_fermat_42 a0 b0 c0) ∧ a0 % 2 = 1 ∧ 0 < c0  :=
begin
  obtain ⟨a0, b0, c0, hf, hc⟩ := exists_odd_minimal_fermat_42 h,
  by_cases h0 : 0 < c0,
  { use [a0, b0, c0], tauto },
  { use [a0, b0, -c0],
    apply and.intro (neg_of_minimal_fermat_42 hf),
    apply and.intro hc,
    apply neg_pos.mpr, apply not_le.mp, intro h2,
    have h3 : 0 < c0, { exact lt_of_le_of_ne h2 (ne.symm hf.1.2.2.1) },
    revert h3, exact h0 }
end
...
begin
  dunfold int.gcd,
  rw int.nat_abs_mul,
  apply nat.coprime.mul_right (coprime_of_sqr_sum h h2),
  rw add_comm at h,
...
begin
  ...
  have hcp : int.gcd m (r * s) = 1, exact coprime_of_sqr_sum' htt3 htt4,
  ...
  { apply int.coe_nat_lt.mp, rw ← (int.eq_nat_abs_of_zero_le (le_of_lt hc)),
    apply gt_of_gt_of_ge _ (abs_le_mul_self i),
    rw [← hi, ht3],
    apply gt_of_gt_of_ge _ (pos_le_mul_self m ht6),
    exact lt_add_of_pos_right (m ^ 2) (pow_two_pos_of_ne_zero n hn) },
  have hic' : int.nat_abs c ≤ int.nat_abs i,
  { apply (h.2 j k i),
    exact and.intro hj0 (and.intro hk0 (and.intro hi0 hh.symm)) },
