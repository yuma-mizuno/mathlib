...
/--If `is_unit n` in a nontrivial `comm_ring R`,
then ` X ^ n - u` is separable for any unit `u`. -/
lemma separable_X_pow_sub_C_unit {R : Type*} [comm_ring R] [nontrivial R] {n : ℕ}
  (u : units R) (hn : is_unit (n : R)) :
  separable (X ^ n - C (u : R)) :=
begin
...
lemma X_pow_sub_one_separable_iff {R : Type*} [comm_ring R] [nontrivial R] {n : ℕ} :
