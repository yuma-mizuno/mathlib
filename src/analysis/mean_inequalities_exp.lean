...
begin
  ...
  { simp_rw [f', div_rpow, ← sum_div, ← rpow_mul, one_div, inv_mul_cancel hpq.ne_zero, rpow_one,
      div_self hF_zero], },
  { simp_rw [g', div_rpow, ← sum_div, ← rpow_mul, one_div, inv_mul_cancel hpq.symm.ne_zero, rpow_one,
      div_self hG_zero], },
end
...
lemma inner_le_Lp_mul_Lq_of_nonneg2 {x₁ x₂ y₁ y₂ : ℝ} (hpq : is_conjugate_exponent p q)
  (hx₁ : 0 ≤ x₁) (hx₂ : 0 ≤ x₂) (hy₁ : 0 ≤ y₁) (hy₂ : 0 ≤ y₂) :
  x₁ * y₁ + x₂ * y₂ ≤ (x₁ ^ p + x₂ ^ p) ^ (1 / p) * (y₁ ^ q + y₂ ^ q) ^ (1 / q) :=
begin
  let s : finset ℕ := {1, 2},
  let f : ℕ → ℝ := λ i, ite (i = 1) x₁ (ite (i = 2) x₂ 0),
  let g : ℕ → ℝ := λ i, ite (i = 1) y₁ (ite (i = 2) y₂ 0),
  have hi_two_of_not_one : ∀ i ∈ s, i ≠ 1 → i = 2,
  { intros i his hi,
    rw finset.mem_insert at his,
    simp only [hi, false_or, mem_singleton] at his,
    exact his, },
  have hf : ∀ i ∈ s, 0 ≤ f i,
  { intros i his,
    simp_rw f,
    by_cases hi : i = 1,
    { simp [hi, hx₁], },
    { have hi2 : i = 2, from hi_two_of_not_one i his hi,
      simp [hi2, hx₂], }, },
  have hg : ∀ i ∈ s, 0 ≤ g i,
  { intros i his,
    simp_rw g,
    ...
    { have hi2 : i = 2, from hi_two_of_not_one i his hi,
      simp [hi2, hy₂], }, },
  calc x₁ * y₁ + x₂ * y₂ = ∑ i in s, f i * g i : by simp [f, g, s]
  ... ≤ (∑ i in s, (f i)^p) ^ (1 / p) * (∑ i in s, (g i)^q) ^ (1 / q) :
    by { refine inner_le_Lp_mul_Lq_of_nonneg s hpq hf hg, }
  ... = (x₁ ^ p + x₂ ^ p) ^ (1 / p) * (y₁ ^ q + y₂ ^ q) ^ (1 / q) : by simp [f, g, s],
end
