...
lemma is_cycle.exists_pow_pow_eq_one {f : perm β} (hf : is_cycle f) :
  ∃ (k : ℕ) (hk : 0 < k), f ^ k = 1 :=
begin
  obtain ⟨x, hx, hx'⟩ := hf,
  by_cases H : f (f x) = x,
  { use 2,
    ...
    { ext y,
      by_cases hy : f y = y,
      { simp [pow_succ, hy] },
      { obtain ⟨k, rfl⟩ := hx' y hy,
        have : f ^ (2 : ℤ) = f ^ 2 := rfl,
        rw [←mul_apply, ←this, ((commute.refl f).gpow_gpow 2 k).eq, mul_apply, this],
        simp [pow_succ, H] } } },
  { by_cases H' : f (f x) = f x,
    { simp only [apply_eq_iff_eq] at H',
      simpa [H'] using H },
    { obtain ⟨k, hk⟩ := hx' _ H',
      rcases lt_trichotomy k 0 with hk'|rfl|hk',
      { have : k < 1 := hk'.trans zero_lt_one,
        use ((-(-1 + k)).to_nat),
        rw [←gpow_coe_nat, int.to_nat_of_nonneg, gpow_neg, gpow_add, inv_eq_one],
        { split,
          { simpa using this },
          { ext y,
            by_cases hy : f y = y,
            { simp [gpow_apply_eq_self_of_apply_eq_self hy, hy, inv_eq_iff_eq] },
            { obtain ⟨m, rfl⟩ := hx' y hy,
              rw [←mul_apply, ←mul_apply, mul_assoc, ((commute.refl f).gpow_gpow _ m).eq, mul_apply,
                  mul_apply, hk, one_mul, ←mul_apply, ←mul_apply, ←gpow_add, ←gpow_add_one,
                  add_comm _ m, int.add_neg_one, sub_add_cancel] } } },
        { refine le_of_neg_le_neg _,
          simpa using this.le } },
      { simp only [perm.coe_one, id.def, gpow_zero] at hk,
        simpa [hk.symm] using hx },
      { have : 1 < k,
        { contrapose! H, },
...
lemma is_cycle.exists_pow_eq {f : perm β} (hf : is_cycle f) {x y : β}
...
begin
  obtain ⟨n, hn⟩ := hf.exists_gpow_eq hx hy,
  use (n % order_of f).to_nat,
  rw [←gpow_coe_nat, int.to_nat_of_nonneg, ←gpow_eq_mod_order_of, hn],
  refine int.mod_nonneg _ (ne_of_gt _),
  rw order_of,
  simp only [int.coe_nat_pos],
  refine is_periodic_pt.minimal_period_pos _ _,
  -- by_cases hx' : f (f x) = x,
  -- {  },
  -- {  },
  -- rw [int.coe_nat_pos],
