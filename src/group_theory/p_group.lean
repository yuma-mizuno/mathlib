...
lemma to_map {H : subgroup G} (hH : is_p_group p H) {K : Type*} [group K]
  (ϕ : G →* K) : is_p_group p (H.map ϕ) :=
begin
  rw [←H.subtype_range, monoid_hom.range_eq_map, subgroup.map_map, ←monoid_hom.range_eq_map],
...
begin
  ...
  have key := subgroup.equiv_map_of_injective,
  have key := of_injective,
end
