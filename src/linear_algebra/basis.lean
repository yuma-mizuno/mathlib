...
{ to_fun := λ f, (finsupp.total M' M' R id).comp $ (finsupp.lmap_domain R R f).compₗ b.repr,
  inv_fun := λ f i, f (b i),
...
theorem constr_def (f : ι → M') :
  b.constr S f = (finsupp.total M' M' R id).compₗ ((finsupp.lmap_domain R R f).compₗ b.repr) :=
rfl
