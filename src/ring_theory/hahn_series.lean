...
lemma is_pwo_Union_support_powers {x : hahn_series Γ R} (hx : 0 < add_val Γ R x) :
...
begin
  ...
  { intros g hn,
    simp only [support_one, set.mem_singleton_iff, pow_zero] at hn,
    rw hn,
    exact add_submonoid.zero_mem _ },
  { intros g hn,
    simp only [pow_succ] at hn,
    obtain ⟨i, j, hi, hj, rfl⟩ := support_mul_subset_add_support hn,
    exact add_submonoid.add_mem _ (add_submonoid.subset_closure hi) (ih hj) }
end

def powers [linear_ordered_add_comm_group Γ] [integral_domain R]
  (x : hahn_series Γ R) (hx : 0 < add_val Γ R x) :
  summable_family Γ R ℕ :=
{ to_fun := λ n, x ^ n,
  is_pwo_Union_support' := is_pwo_Union_support_powers hx,
  co_support := λ g, begin
    have hpwo : (⋃ n : ℕ, {g | (x ^ n).coeff g ≠ 0 }).is_pwo := is_pwo_Union_support_powers hx,
    by_cases hg : g ∈ ⋃ n : ℕ, {g | (x ^ n).coeff g ≠ 0 },
    { apply hwf.induction hg,
      intros y ys hy,
      refine (((add_antidiagonal x.is_wf_support hwf y).finite_to_set.bUnion (λ ij hij,
        hy ij.snd _ _)).union (set.finite_singleton 0)).subset _,
      { rw [mem_coe, mem_add_antidiagonal] at hij,
        exact hij.2.2 },
      { rw [mem_coe, mem_add_antidiagonal] at hij,
        obtain ⟨rfl, hi, hj⟩ := hij,
        have h : 0 < ij.fst :=
          with_top.coe_lt_coe.1 (lt_of_lt_of_le hx (add_val_le_of_coeff_ne_zero hi)),
        by_contra con,
        have h := add_lt_add_of_lt_of_le h (le_of_not_lt con),
        rwa [zero_add, lt_self_iff_false] at h },
      { intros n hn,
        cases n,
        { exact (set.subset_union_right _ _) (set.mem_singleton 0) },
        { rw [set.mem_set_of_eq, pow_succ] at hn,
          obtain ⟨i, j, hi, hj, rfl⟩ := support_mul_subset_add_support hn,
          apply (set.subset_union_left _ _),
          refine set.mem_Union.2 ⟨⟨i, j⟩, set.mem_Union.2 ⟨_, _⟩⟩,
