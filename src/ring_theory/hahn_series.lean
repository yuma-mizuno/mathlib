...
@[simp, to_additive]
lemma finprod_emb_domain {M : Type*} [comm_monoid M] (f : α ↪ β) (g : α → M) :
  ∏ᶠ (b : β), (if h : b ∈ set.range f then g (classical.some h) else 1) =
    ∏ᶠ (a : α), g a :=
begin
...
begin
  ...
  refine finsum_emb_domain f (λ a, (s a).coeff g)
end
...
lemma map_le_sub {g : with_top Γ} {x y : hahn_series Γ R}
  (hx : g ≤ (add_val Γ R) x) (hy : g ≤ (add_val Γ R) y) : g ≤ (add_val Γ R) (x - y) :=
  sorry

lemma foo (x : hahn_series Γ R) (x0 : x ≠ 0) :
